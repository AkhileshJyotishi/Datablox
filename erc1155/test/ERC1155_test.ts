...
describe("DatasetOwnership", function () {
  ...
  beforeEach(async function () {
    const [ownerSigner, buyerSigner] = await ethers.getSigners();
  ...
  it("should allow a buyer to purchase partial ownership and set expiry", async function () {
    ...
    await datasetOwnership.connect(await ethers.getSigner(buyer)).buyPartialOwnership(1, purchaseDuration, { value: ethers.parseEther("1") });
  ...
  it("should revert getDatasetUri if caller does not own the token", async function () {
    ...
    await expect(
      datasetOwnership.connect(await ethers.getSigner(buyer)).getDatasetUri(1)
    ).to.be.revertedWith("Access denied: you do not own this dataset");
  });
