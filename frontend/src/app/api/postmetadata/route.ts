...
export async function POST(request: Request) {
  try {
    ...
    const metaDataResponse = await supabase.from("metadata").upsert(metadata).select()
    ...
    const authorResponse = await supabase.from("publisher").select("*").eq("id", owner)

    if (authorResponse.error) {
      console.error(authorResponse.error)
      return NextResponse.json({ error: authorResponse.error.message }, { status: 500 })
    }
    ...
    if (authorResponse.data && authorResponse.data.length > 0) {
      ...
      const updateResponse = await supabase.from("publisher").update({ metaDataId: updatedMetaDataIds }).eq("id", owner)
      ...
      const insertResponse = await supabase.from("publisher").insert({ id: owner, metaDataId: [metadataId] })
    ...
    for (const tag of tags) {
      ...
      const { data: tagData, error: tagError } = await supabase.from("tags").select("*").eq("id", tag)
      ...
      if (tagData && tagData.length > 0) {
        ...
        if (!existingMetadataIds.includes(metadataId)) {
          ...
          if (updateError) {
            ...
            tagErrors.push(`Error updating tag "${tag}": ${updateError.message}`)
          } else {
        ...
        const { error: insertError } = await supabase.from("tags").insert({ id: tag, metadataid: [metadataId] })

        if (insertError) {
          console.error(`Error inserting new tag "${tag}":`, insertError)
          tagErrors.push(`Error inserting new tag "${tag}": ${insertError.message}`)
        } else {
    ...
    return NextResponse.json({ message: "Metadata processed successfully", metadataId }, { status: 200 })
  } catch (err: any) {
