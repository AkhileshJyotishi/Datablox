...
export function Sidebar({ collapsed = false, onCollapse }: SidebarProps) {
  ...
  return (
    <div
      className={`flex rounded-xl h-full flex-col border border-[#303030] bg-transparent backdrop-blur-sm text-white transition-all duration-300 ${
        collapsed ? "w-16" : "w-64"
      }`}
    >
      <div className="flex h-16 items-center border-b border-zinc-700 px-4">
        <button className="flex w-full items-center gap-2 rounded-lg px-2 py-1 hover:bg-zinc-800" onClick={onCollapse}>
          <FileSpreadsheet className="h-5 w-5 text-[#9e2750]" />
          {!collapsed && <span className="font-bold truncate">Dataset Generator</span>}
        </button>
      </div>
